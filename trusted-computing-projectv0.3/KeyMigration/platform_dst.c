...
gen_pubSrk(void)
{
	...
	// Get SRK Pub Key
	result = Tspi_TPM_OwnerGetSRKPubKey(hTPM, &u32PubSrkLen, &prgbPubSrk);//获得SRK公钥，存于prgbPubSrk
	if (TSS_SUCCESS != result) {
	...
	len = fwrite(&u32PubSrkLen, sizeof(u32PubSrkLen), 1, fp);//写入长度
	if (1 != len) {
	...
	len = fwrite(prgbPubSrk, u32PubSrkLen, 1, fp);//写入公钥
	if (1 != len) {
...
mig_key(void)
{
	...
	result = Tspi_Context_CreateObject(hContext, 
					...
					TSS_KEY_MIGRATABLE, 
					&hNewMigKey);//hNewMigKey 准备接受迁移的密钥
	if (TSS_SUCCESS != result) {
	...
	// TODO: 完成以下代码，参考 Tspi_Key_ConvertMigrationBlob
	//从 mig.blob中已经获取了 u32RandomLen, pRandom,u32MigBlobLen,pMigBlob
