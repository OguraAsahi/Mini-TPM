...
#define KEY_SIZE 64
	...
	result = Tspi_Context_Create(&hContext);   //首先生成上下文环境, 加载秘钥, 获得秘钥策略
	...
	result = Tspi_TPM_GetRandom(hTPM, KEY_SIZE, &random);  //调用Random函数通过TPM的随机数发生器生成随机数random,作为对称加密秘钥
	if (TSS_SUCCESS != result) {
	...
	result = Tspi_TPM_PcrRead(hTPM, 15, &u32PcrValLen, &rgbPcrVal);// 获取PCR15的值存于rgbPcrVal中
	if (TSS_SUCCESS != result) {
	...
	result = Tspi_PcrComposite_SetPcrValue(hPcrs, 15, u32PcrValLen, rgbPcrVal); //将PCR15的值与PCR句柄hPcrs绑定
	if (TSS_SUCCESS != result) {
	...
	result = Tspi_Data_Seal(hEncData, 
					...
					KEY_SIZE, 
					random, 
					hPcrs); //调用Data_Seal函数进行seal操作,用K1加密对称密钥random,与PCR15绑定后封装,获得句柄hEncData
	if (TSS_SUCCESS != result) {
	...
	result = Tspi_GetAttribData(hEncData, 
						...
						&u32EncDataLen, 
						&rgbEncData); //调用GetAttributeData获得hEncData中秘密数据的长度和内容,存于u32EncDataLen和rgbEncData,准备完毕
	if (TSS_SUCCESS != result) {
	...
	fseek(fpIn, 0, SEEK_END); //获得文件长度size
	size = ftell(fpIn);
	...
	pBufIn = malloc(size);//准备把文件内容读入缓冲区pBufIn中
	if (!pBufIn) {
		printf("No Memmory\n");
		Tspi_Context_Close(hContext);
	}
	...
	if (aes_init(random, KEY_SIZE, (unsigned char *)&salt, &en)) { //初始化AES加密, 加入了秘钥random
	...
	pBufOut = aes_encrypt(&en, pBufIn, &size); //将pBufIn中的数据用random加密,写入到pBufOut

	fpOut = fopen(argv[2], "wb");// 打开密文文件
	if (!fpOut) {
		printf("open file: %s failed\n", argv[2]);
		Tspi_Context_Close(hContext);
		free(pBufIn);
	...
	len = fwrite(&u32EncDataLen, 1, sizeof(UINT32), fpOut); //向密文文件中写入封装信息长度u32EncDataLen
	...
	len = fwrite(rgbEncData, 1, u32EncDataLen, fpOut); //向密文文件中写入封装信息内容rgbEncData
	...
	len = fwrite(&size, 1, sizeof(int), fpOut); //向密文文件中写入文件大小size
	if (len != sizeof(int)) {
	...
	len = fwrite(pBufOut, 1, size, fpOut); //向密文文件中写入文件密文
	if (len != size) {
		printf("fwrite failed\n");
		Tspi_Context_Close(hContext);
		free(pBufIn);
